    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-72 bg-card shadow-xl border-r border-border flex flex-col">
                ...
                <div class="bg-accent rounded-lg p-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-muted-foreground">{{ auth()->user()->roles->first()->name ?? 'Sin rol' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 p-4 overflow-y-auto">
                ...
                <!-- Proyectos Section -->
                @if(auth()->user()->can('projects.index') || auth()->user()->can('projects.create') || auth()->user()->can('projects.report'))
                <div class="mb-6">
                    <h3 class="sidebar-section-title text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
                        Proyectos
                    </h3>
                            ...
                            <li>
                                <a href="{{ route('projects.index') }}" 
                                   class="sidebar-item flex items-center px-4 py-3 text-sm rounded-lg {{ request()->routeIs('projects.index') ? 'active' : 'text-muted-foreground hover:text-foreground' }}">
                                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                    Ver Proyectos
                                </a>
                            </li>
                        ...
                        @can('projects.report')
                            <li>
                                <a href="{{ route('projects.report') }}" 
                                   class="sidebar-item flex items-center px-4 py-3 text-sm rounded-lg {{ request()->routeIs('projects.report') ? 'active' : 'text-muted-foreground hover:text-foreground' }}">
                                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                    Reportes
                        ...
                        @can('events.create')
                            <li>
                                <a href="{{ route('events.create') }}" 
                                   class="sidebar-item flex items-center px-4 py-3 text-sm rounded-lg {{ request()->routeIs('events.create') ? 'active' : 'text-muted-foreground hover:text-foreground' }}">
                                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                    </svg>
                                    Crear Evento
                    ...
                    <div class="max-w-7xl mx-auto">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                            ...
                            <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-foreground">{{ \App\Models\Semillero::count() ?? 0 }}</p>
                                    </div>
                            ...
                            <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm text-muted-foreground">Integrantes Activos</p>
                                        <p class="text-2xl font-bold text-foreground">{{ \App\Models\User::count() ?? 0 }}</p>
                                    </div>
                                    <div class="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center">
                                        <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"/>
                            ...
                            <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-foreground">{{ \App\Models\Project::count() ?? 0 }}</p>
                                    </div>
                                    <div class="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center">
                                        <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="bg-card rounded-lg p-6 shadow-sm border border-border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-foreground">{{ \App\Models\Event::count() ?? 0 }}</p>
                                    </div>
                                    <div class="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center">
                                        <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2z"/>
                        ...
                        <div class="bg-card rounded-lg p-8 shadow-sm border border-border">
                            <div class="text-center">
                                ...
                                <p class="text-lg text-muted-foreground mb-6">Sistema de Gestión y Seguimiento de Grupos de Semilleros de Investigación</p>
                                <div class="flex justify-center space-x-4">
                                    @can('semilleros.create')
                                        <a href="{{ route('semilleros.create') }}" class="bg-primary hover:bg-primary/90 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                                            Crear Semillero
                                        </a>
                                    @endcan
                                    @can('projects.create')
                                        <a href="{{ route('projects.create') }}" class="bg-secondary hover:bg-secondary/80 text-secondary-foreground px-6 py-3 rounded-lg font-medium transition-colors">
                                            Nuevo Proyecto
                                        </a>
                                    @endcan
                                </div>

...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-card rounded-lg shadow-sm border border-border overflow-hidden">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <form id="searchForm" action="{{ route('semilleros.index') }}" method="GET" class="mt-2 mb-2">
                            <div class="flex items-center space-x-2">
                                <div class="relative">
                                    <svg class="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {{-- Tabla rediseñada con estilo SENA profesional y botones de acción mejorados --}}
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-border">
                <thead class="bg-muted/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">#</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Semillero</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Descripción</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Fecha Creación</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider">Acciones</th>
                    </tr>
                </thead>
                <tbody id="semillerosTable" class="bg-card divide-y divide-border">
                    @forelse ($semilleros as $index => $semillero)
                        <tr class="hover:bg-muted/30 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{{ $loop->iteration }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{{ $semillero->created_at->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    @can('semilleros.create')
                                        ...
                                        <form action="{{ route('semilleros.destroy', $semillero) }}" method="POST" class="inline" onsubmit="return confirm('¿Seguro que deseas eliminar este semillero?');">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center space-y-4">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-medium text-foreground mb-1">No hay semilleros registrados</h3>
                                        <p class="text-muted-foreground mb-4">Comienza creando tu primer semillero de investigación</p>
                                        @can('semilleros.create')
                                            <a href="{{ route('semilleros.create') }}"
        ...
        {{-- Paginación mejorada con diseño SENA y queryString --}}
        @if($semilleros->hasPages())
            <div id="pagination" class="px-6 py-4 border-t border-border bg-muted/30">
                {{ $semilleros->withQueryString()->links() }}
            </div>
        @endif
    </div>
</div>
...
{{-- Script búsqueda + paginación AJAX --}}
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const table = document.getElementById('semillerosTable');
            ...
            function ajaxLoad(url) {
                ...
                fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }, signal: controller.signal })
                    ...
                    .then(html => {
                        ...
                        const newTable = doc.getElementById('semillerosTable');
            ...
            function bindPaginationLinks() {
                document.querySelectorAll('#pagination a').forEach(a => {
                    a.addEventListener('click', e => {
                        e.preventDefault(); ajaxLoad(a.href);
                    });
                });
            }
            ...
            if (input) {
                input.addEventListener('input', () => {
                    clearTimeout(timer);
                    timer = setTimeout(() => ajaxLoad(buildUrl()), 300);
                });
            }
            bindPaginationLinks();
        });
